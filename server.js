...
var server = http.createServer(function (request, response) {
  ...
  if (path === "/") {
    ...
    string = string.replace("{{json}}", `<ul id = 'list'>${result}</ul>`);
    ...
    response.statusCode = 200;
    response.setHeader("Content-Type", "text/html;charset=utf-8");
    const string = fs.readFileSync("src/test.html");
    response.write(string);
    response.end();
  } else if (path === "/test.css") {
    response.statusCode = 200;
    response.setHeader("Content-Type", "text/css;charset=utf-8");
    const string = fs.readFileSync("src/test.css");
    response.write(string);
    response.end();
  } else if (path === "/test.js") {
    response.statusCode = 200;
    response.setHeader("Content-Type", "text/css;charset=utf-8");
    const string = fs.readFileSync("src/test.js");
    response.write(string);
    response.end();
  } else if (path === "/test.xml") {
    response.statusCode = 200;
    response.setHeader("Content-Type", "text/xml;charset=utf-8");
    const string = fs.readFileSync("src/test.xml");
    response.write(string);
    response.end();
  } else if (path === "/test.json") {
    response.statusCode = 200;
    response.setHeader("Content-Type", "text/json;charset=utf-8");
    const string = fs.readFileSync("src/test.json");
    response.write(string);
    response.end();
